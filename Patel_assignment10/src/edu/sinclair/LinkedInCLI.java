...
public class LinkedInCLI {
	...
	/**
	 * 
	 * EXPLANATION - I have used Stack data structure for undoing remove connections and add connections. Also i have created 
	 * UndoAction Class which has properties linkedInUser,option, description.
	 * The Stack is based on the basic principle of last-in-first-out.
	 * In addition to the basic push and pop operations, the Stack provides more functions of empty, search and peek. 
	 * 
	 */
	...
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (option == 4) {
				...
				ex.listSkillsets();
			}else if (option == 14) {// Multiundo
					ex.multiUndo(input);
			} else if (option == 15) { // sign off
				ex.undoActions = new Stack<>();
				ex.signOff();
				ex.establishUser(input);
			} else if (option == 16) { // quit
				ex.quit();
				System.exit(0);
				input.close();
			}
	...
	public void multiUndo(Scanner input) {
		if(undoActions.size() == 0) {
			System.out.println("There are no actions to undo. Please do an undoable operation first.");
			return;
		}
		UndoAction action = undoActions.peek();
		System.out.println("The last menu option selected was " + action.getDescription() + " involving " + action.getLinkedInUser().getUsername() + ". Undo (Y/N)");
		String confirm = input.next();
		if(confirm.equals("Y")) {
			try {
				if(action.getOption() == 8) {
					// remove here
					loggedInUser.removeConnection(action.getLinkedInUser());
					undoActions.pop();
					System.out.println("Removed Connection " + action.getLinkedInUser().getUsername() + " from connections.");
				} else {
					//add here.
					loggedInUser.addConnection(action.getLinkedInUser());
					undoActions.pop();
					System.out.println("Added Connection " + action.getLinkedInUser().getUsername() + " back");
				}
	...
	public int displayMenu(Scanner input) {
		...
		System.out.println("15. Sign Off");
	...
	private void addConnection(Scanner input) {
		...
		if (index < 0) {
			...
			try {
				...
				UndoAction action = new UndoAction(exists, 8, "Add Connection");
	...
	private void removeConnection(Scanner input) {
		...
		if (index < 0) {
			...
			try {
				...
				UndoAction action = new UndoAction(exists, 9, "Remove Connection");
				undoActions.push(action);
			} catch (LinkedInException e) {
				System.out.println(e.getMessage());
			}
